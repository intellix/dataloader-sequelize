...
import { EXPECTED_OPTIONS_KEY, BATCH_BY_ATTRIBUTE, createContext, removeContext } from '../../src';
...
describe('findOne', function () {
  ...
  describe('id primary key', function () {
    ...
    it('works with null', async function () {
      const user0 = await this.User[this.method]({ [EXPECTED_OPTIONS_KEY]: this.context });
  ...
  describe('other fields', function () {
    ...
    it('works with other where clauses', async function () {
      ...
      expect(this.User.findAll, 'to have a call satisfying', [{
        where: {
          id: [this.user1.get('id')],
          status: this.user1.get('status'),
        }
      }]);
    });

    it('batches to a single findAll call with same extra conditions', async function () {
      const user1 = this.User[this.method]({ where: { id: this.user1.get('id'), status: 'active' }, [EXPECTED_OPTIONS_KEY]: this.context });
      const user3 = this.User[this.method]({ where: { id: this.user3.get('id'), status: 'active' }, [EXPECTED_OPTIONS_KEY]: this.context });
      ...
      expect(this.User.findAll, 'to have a call satisfying', [{
        where: {
          id: [this.user1.get('id'), this.user3.get('id')],
          status: 'active',
        }
      }]);
    });

    it('does not batch with different extra conditions', async function () {
      const user1 = this.User[this.method]({ where: { id: this.user1.get('id'), status: this.user1.get('status') }, [EXPECTED_OPTIONS_KEY]: this.context });
      const user2 = this.User[this.method]({ where: { id: this.user2.get('id'), status: this.user2.get('status') }, [EXPECTED_OPTIONS_KEY]: this.context });
      ...
      expect(this.User.findAll, 'to have a call satisfying', [{
        where: {
          id: [this.user2.get('id')],
          status: this.user2.get('status'),
        }
      }]);
    });

    it('falls through without batching when where contains Op symbols', async function () {
      const Op = Sequelize.Op;
      if (!Op) return; // Op not available in Sequelize v3 - skip

      const user1 = this.User[this.method]({
        where: { id: this.user1.get('id'), [Op.and]: [{ status: 'active' }] },
        [EXPECTED_OPTIONS_KEY]: this.context
      });
      const user3 = this.User[this.method]({
        where: { id: this.user3.get('id'), [Op.and]: [{ status: 'active' }] },
        [EXPECTED_OPTIONS_KEY]: this.context
      });
      ...
      // Both calls share the same Op conditions — should batch into a single findAll
      expect(this.User.findAll, 'was called once');
      expect(this.User.findAll, 'to have a call satisfying', [{
        where: {
          id: [this.user1.get('id'), this.user3.get('id')],
    ...
    it('does not batch with different Op symbol conditions', async function () {
      const Op = Sequelize.Op;
      if (!Op) return; // Op not available in Sequelize v3 - skip
      ...
      const user2 = this.User[this.method]({
        where: { id: this.user2.get('id'), [Op.and]: [{ status: 'inactive' }] },
        [EXPECTED_OPTIONS_KEY]: this.context
      });
      ...
      // Different Op conditions — should use separate loaders
      expect(this.User.findAll, 'was called twice');
      expect(this.User.findAll, 'to have a call satisfying', [{
        where: { id: [this.user1.get('id')], [Op.and]: [{ status: 'active' }] }
      }]);
    ...
    it('batches calls with the same order', async function () {
      const user1 = this.User[this.method]({
        where: { id: this.user1.get('id'), status: 'active' },
        order: [['id', 'DESC']],
        [EXPECTED_OPTIONS_KEY]: this.context
      });
      const user3 = this.User[this.method]({
        where: { id: this.user3.get('id'), status: 'active' },
        order: [['id', 'DESC']],
        [EXPECTED_OPTIONS_KEY]: this.context
      });
      ...
      expect(this.User.findAll, 'was called once');
      expect(this.User.findAll, 'to have a call satisfying', [{
        where: { id: [this.user1.get('id'), this.user3.get('id')], status: 'active' },
        order: [['id', 'DESC']]
      }]);
    });

    it('does not batch calls with different order', async function () {
      const user1 = this.User[this.method]({
        where: { id: this.user1.get('id'), status: 'active' },
        order: [['id', 'ASC']],
        [EXPECTED_OPTIONS_KEY]: this.context
  ...
  describe('primary key with field', function () {
    ...
    it('batches to a single findAll call', async function () {
      ...
      const user2 = this.User[this.method]({ where: { id: this.user2.get('id') }, [EXPECTED_OPTIONS_KEY]: this.context });
  ...
  describe('BATCH_BY_ATTRIBUTE', function () {
    beforeEach(createConnection);
    beforeEach(async function () {
      this.sandbox = sinon.sandbox.create();
      ...
      [this.user1, this.user2, this.user3] = await this.User.bulkCreate([
        { id: randint(), parentId: randint(), status: 'active' },
        { id: randint(), parentId: randint(), status: 'active' },
        { id: randint(), parentId: randint(), status: 'inactive' }
      ], { returning: true });
    ...
    it('batches by the specified attribute', async function () {
      const user1 = this.User[this.method]({
        where: { parentId: this.user1.get('parentId'), status: 'active' },
        [BATCH_BY_ATTRIBUTE]: 'parentId',
        [EXPECTED_OPTIONS_KEY]: this.context
      });
      const user2 = this.User[this.method]({
        where: { parentId: this.user2.get('parentId'), status: 'active' },
        [BATCH_BY_ATTRIBUTE]: 'parentId',
        [EXPECTED_OPTIONS_KEY]: this.context
      });
      ...
      expect(this.User.findAll, 'to have a call satisfying', [{
        where: {
          parentId: [this.user1.get('parentId'), this.user2.get('parentId')],
          status: 'active'
        }
      }]);
    });

    it('does not batch when extra conditions differ', async function () {
      const user1 = this.User[this.method]({
        where: { parentId: this.user1.get('parentId'), status: 'active' },
        [BATCH_BY_ATTRIBUTE]: 'parentId',
        [EXPECTED_OPTIONS_KEY]: this.context
      ...
      const user3 = this.User[this.method]({
        where: { parentId: this.user3.get('parentId'), status: 'inactive' },
        [BATCH_BY_ATTRIBUTE]: 'parentId',
        [EXPECTED_OPTIONS_KEY]: this.context
      });
    ...
    it('respects order option', async function () {
      const user1 = this.User[this.method]({
        where: { parentId: this.user1.get('parentId'), status: 'active' },
        order: [['id', 'DESC']],
        [BATCH_BY_ATTRIBUTE]: 'parentId',
    ...
    it('supports rejectOnEmpty', async function () {
      const missing = this.User[this.method]({
        where: { parentId: 999999, status: 'active' },
        rejectOnEmpty: true,
        [BATCH_BY_ATTRIBUTE]: 'parentId',
        [EXPECTED_OPTIONS_KEY]: this.context
      });
      const found = this.User[this.method]({
        where: { parentId: this.user1.get('parentId'), status: 'active' },
        [BATCH_BY_ATTRIBUTE]: 'parentId',
        [EXPECTED_OPTIONS_KEY]: this.context
      });

      await expect(missing, 'to be rejected');
      await expect(found, 'to be fulfilled with', this.user1);
    });
  });
});
